...
public struct Pace: Hashable, Sendable {
    public struct Mover: Hashable, Sendable, Identifiable {
        public let category: SpendCategory
...
public enum PaceEngine {
    public static func pace(transactions: [Transaction], streams: [RecurringStream], now: Date, calendar: Calendar = .current) -> Pace {
        ...
        let wants = thisRows.filter { isSpend($0) && ($0.category?.lens ?? .wants) == .wants }.reduce(0) { $0 + $1.magnitude }
        let needs = thisRows.filter { isSpend($0) && $0.category?.lens == .needs }.reduce(0) { $0 + $1.magnitude }
        ...
        var byCat: [SpendCategory: (Decimal, Decimal)] = [:]
        for t in thisRows where isSpend(t) { byCat[t.category ?? .other, default: (0, 0)].0 += t.magnitude }
        for t in lastRows where isSpend(t) { byCat[t.category ?? .other, default: (0, 0)].1 += t.magnitude }
        let movers = byCat.map { Pace.Mover(category: $0.key, thisMonth: $0.value.0, lastMonth: $0.value.1) }
